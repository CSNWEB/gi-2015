...
private:
    ...
    /*!
     *  Prepare index set for computation of convex hull: for each x-coordinate existing (respecting tolerance), delete every point which y-coordinate is not minimum or maximum for this x-coordinate. Thus removing all points that are not relevant to the convex hull.
     *
     *  @param points       A pointer to a vector of Point, for which the index-vector should be trimmed.
     *  @param indices      A sorted vector of ints that containes all indices of points in the vector points.
     *
     *  @return             True, if points have been deleted. False otherwise.
     */
    static bool trim_indices_of_points_in_y(vector<Point> &points, vector<int> &indices);
